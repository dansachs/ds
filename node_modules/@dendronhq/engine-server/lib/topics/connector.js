...
class EngineConnector {
    constructor({ wsRoot, logger }) {
        this.wsRoot = wsRoot;
        this.logger = logger || common_server_1.createLogger("connector");
    ...
    async tryToConnect({ port }) {
        ...
        if (!lodash_1.default.isUndefined(resp.error)) {
    ...
    async _connect(opts) {
        const portFilePath = utils_1.getPortFilePath(opts);
        const metaFpath = utils_1.getWSMetaFilePath(opts);
        ...
        if (portCreated > wsActivation) {
            const port = utils_1.openPortFile({ fpath: portFilePath });
