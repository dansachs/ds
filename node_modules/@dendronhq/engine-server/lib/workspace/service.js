...
exports.WorkspaceService = exports.SyncActionStatus = void 0;
...
(function (SyncActionStatus) {
    ...
    SyncActionStatus["UNCOMMITTED_CHANGES"] = "it has uncommitted changes";
...
class WorkspaceService {
    constructor({ wsRoot, seedService }) {
        this.wsRoot = wsRoot;
        this.logger = common_server_1.createLogger();
        this.seedService = seedService || new seed_1.SeedService({ wsRoot });
    }
    ...
    static async isWorkspaceVault(fpath) {
        return await fs_extra_1.default.pathExists(path_1.default.join(fpath, common_all_1.CONSTANTS.DENDRON_CONFIG_FILE));
    }
    get user() {
    ...
    get config() {
        return config_1.DConfig.defaults(config_1.DConfig.getOrCreate(this.wsRoot));
    }
    get dendronRoot() {
    ...
    setWorkspaceConfig(config) {
        common_server_1.writeJSONWithComments(path_1.default.join(this.wsRoot, "dendron.code-workspace"), config);
    }
    /**
     *
     * @param param0
    ...
    async addVault(opts) {
        ...
        if (!config.site.duplicateNoteBehavior) {
            config.site.duplicateNoteBehavior = {
                ...
                payload: config.vaults.map((v) => common_all_1.VaultUtils.getName(v)),
            };
        }
        ...
        if (updateWorkspace) {
            ...
            let out = (await common_server_1.readJSONWithComments(wsPath));
            if (!lodash_1.default.find(out.folders, (ent) => ent.path === common_all_1.VaultUtils.getRelPath(vault))) {
                ...
                out = common_server_1.assignJSONWithComment({ folders }, out);
                common_server_1.writeJSONWithComments(wsPath, out);
            }
        }
        return vault;
    }
    ...
    async shouldVaultsSync(command, [root, vaults]) {
        ...
        if (lodash_1.default.isUndefined(config)) {
            if (await WorkspaceService.isWorkspaceVault(root)) {
                ...
                if (lodash_1.default.isUndefined(config))
                    config = common_all_1.DVaultSync.NO_COMMIT;
            }
            // default for regular vaults
            else
                config = common_all_1.DVaultSync.SYNC;
        }
        if (config === common_all_1.DVaultSync.SKIP)
            return false;
        if (config === common_all_1.DVaultSync.SYNC)
            return true;
        if (config === common_all_1.DVaultSync.NO_COMMIT && command === "commit")
            return false;
        if (config === common_all_1.DVaultSync.NO_PUSH && command === "push")
            return false;
        return true;
    }
    ...
    async removeVault(opts) {
        ...
        config.vaults = lodash_1.default.reject(config.vaults, (ent) => {
            const checks = [
        ...
        if (vault.workspace && config.workspaces) {
            const vaultWorkspace = lodash_1.default.find(config.vaults, {
        ...
        if (updateWorkspace) {
            const wsPath = path_1.default.join(this.wsRoot, DENDRON_WS_NAME);
            let settings = (await common_server_1.readJSONWithComments(wsPath));
            const folders = lodash_1.default.reject(settings.folders, (ent) => ent.path === common_all_1.VaultUtils.getRelPath(vault));
            settings = common_server_1.assignJSONWithComment({ folders }, settings);
            common_server_1.writeJSONWithComments(wsPath, settings);
        }
    }
    createConfig() {
        this.config;
    ...
    static async createWorkspace(opts) {
        ...
        fs_extra_1.default.writeFileSync(gitIgnore, ["node_modules", ".dendron.*", "build", "\n"].join("\n"), { encoding: "utf8" });
    ...
    async cloneVault(opts) {
        ...
        const repoPath = common_server_1.vault2Path({ wsRoot, vault });
    ...
    async getVaultRepo(vault) {
        const vpath = common_server_1.vault2Path({ vault, wsRoot: this.wsRoot });
        return await common_server_1.GitUtils.getGitRoot(vpath);
    }
    async getAllReposVaults() {
        const reposVaults = new Map();
        await Promise.all(this.config.vaults.map(async (vault) => {
    ...
    async pullVault(opts) {
        ...
        const git = common_server_1.simpleGit({ baseDir: repoPath });
        await git.pull();
        return repoPath;
    }
    ...
    async removeVaultCaches() {
        await Promise.all(this.config.vaults.map((vault) => {
            return utils_1.removeCache(common_server_1.vault2Path({ wsRoot: this.wsRoot, vault }));
        }));
    }
    /**
    ...
    async syncVaults(opts) {
        ...
        const { wsRoot } = this;
        // check workspaces
        const workspacePaths = (await Promise.all(lodash_1.default.map(config.workspaces, async (wsEntry, wsName) => {
        ...
        // check seeds
        const seedResults = [];
        await Promise.all(lodash_1.default.map(config.seeds, async (entry, id) => {
        ...
        const emptyRemoteVaults = config.vaults.filter((vault) => !lodash_1.default.isUndefined(vault.remote) &&
            !fs_extra_1.default.existsSync(common_server_1.vault2Path({ vault, wsRoot })));
        const didClone = !lodash_1.default.isEmpty(emptyRemoteVaults) ||
            !lodash_1.default.isEmpty(workspacePaths) ||
            !lodash_1.default.isUndefined(seedResults.find((ent) => ent.status === SyncActionStatus.NEW));
        ...
        if (fetchAndPull) {
            const vaultsToFetch = lodash_1.default.difference(config.vaults.filter((vault) => !lodash_1.default.isUndefined(vault.remote)), emptyRemoteVaults);
